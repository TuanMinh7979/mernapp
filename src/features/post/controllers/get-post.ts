...
export class Get {
  public async posts(req: Request, res: Response): Promise<void> {
    try{
      ...
      // first get in redis 
      console.log(newSkip, limit);
      
      const cachedPosts: IPostDocument[] = await postCache.getPostsFromCache('post', newSkip, limit-1);
  ...
  public async postsWithImages(req: Request, res: Response): Promise<void> {
    ...
    const newSkip: number = skip === 0 ? skip : skip + 1;
    let posts: IPostDocument[] = [];
    const cachedPosts: IPostDocument[] = await postCache.getPostsWithImagesFromCache('post', newSkip, limit-1);
